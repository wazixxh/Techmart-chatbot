...
export default function Home() {
  ...
  const sendMessage = async (questionText: string) => {
    ...
    try {
      ...
      const errorMessage = err?.message || 'Failed to connect to the TechMart AI server.';
      setMessages((prev) =>
        prev.map((msg) =>
          msg.id === assistantId
      ...
      <div className="z-10 flex-1 w-full max-w-4xl flex flex-col bg-slate-900/40 backdrop-blur-sm rounded-3xl border border-slate-800/60 shadow-2xl overflow-hidden relative">
            ...
            {isLoading && (
              <div className="flex gap-4 justify-start">
                ...
                <div className="bg-slate-800/80 border border-slate-700/50 rounded-2xl px-5 py-4 flex items-center gap-2">
                  ...
                  <span className="text-slate-400 text-sm">Searching TechMart knowledge base...</span>
                </div>
              </div>
            )}
        ...
        <div className="p-4 bg-slate-900/80 border-t border-slate-800/60 backdrop-blur-md">
          <form
            ...
            className="flex relative items-end bg-slate-950/50 border border-slate-700/50 rounded-2xl overflow-hidden focus-within:border-blue-500/50 focus-within:ring-1 focus-within:ring-blue-500/50 transition-all shadow-inner"
          >
            <textarea
          ...
          <div className="text-center mt-3">
            <p className="text-xs text-slate-500 font-medium">
              Answers are generated from TechMart's policy and product catalog.
            </p>
          </div>
        </div>
      </div>
